...
3. **多物体任意抓取** —— 每轮自动检测多个物体、YLYW 规划后**从中选择一个目标**进行抓取；
   单轮抓取一个目标，不是“检测几个就连续抓几个”。
...
物体深度特征(13维)
  ...
  │  L3+ 爻位关系       —— 当位/得中/乘承比应 → 谨慎度、抓取强度修正×modifier
  ▼
抓取策略 {type, force(抓取强度0~1), approach_angle, speed, cautions}
  │  参数化
  ▼
机械臂抓取参数 {接近方式, 夹爪闭合值, 速度档(1~100), 运动序列}
```

例如合成方块场景得到：
- 卦象 **风雷益**，抓取类型 `progressive_grasp`(从轻到重逐级夹紧)，抓取强度 0.40
- 卦象 **地泽临**，抓取类型 `top_down_grasp`(正上方垂直抓取)，抓取强度 0.45
- 卦象 **火山旅**，抓取类型 `conditional_grasp`(适应环境动态调整)，抓取强度 0.50

> 注：YLYW 的 `force`(0~1) 是**抓取强度/夹爪闭合程度参数**，不是物理夹持力(N)。
> 下发到真机的是 `set_gripper_value` 的夹爪位置/开合值，需按实际夹爪型号与标定确认数值方向。
...
- **速度档已改为 MyCobot 1~100**：`slow/normal/fast = 20/35/50`。
  旧版 `2/3/4` 是 1~5 时代的写法，真机上极慢；初测可整体再压低。
- **夹爪“力”语义**：`force` 是抓取强度(0~1)/夹爪闭合度，**不是牛顿力**；
  下发为 `set_gripper_value(value)` 夹爪开合值。真机夹爪型号与标定决定数值方向，实机前务必确认。
- **抓取高度(Z)防呆**：`GraspController.pick` 会对抓取高度做 [20,500]mm 合理性校验，
  超范围直接报错拒绝执行——防止坐标未标定时 Z 反向/极端值直接驱动真机。
  先做手-眼标定（默认 `from_overhead` 仅占位，工作区约 140–220mm）。
- **接近/抓取 Z 方向**：先到“抓取点上方(高位)张开 → 下降至抓取高度 → 夹紧 → 抬升”，
  approach_z > grasp_z（基座系 Z 朝上为正），已修复旧的 Z 方向颠倒 bug。
- **`--dryrun` 的 success 是“规划成功”**，不是物理抓取成功，其“成功率”≠抓取成功率；
  正式论文须用真机实测。
