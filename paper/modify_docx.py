...
# ============================================================
# Helper: Insert paragraphs before a target paragraph index
# ============================================================
def insert_paragraphs_before(doc, before_idx, new_paras_data):
    """
    Insert new paragraphs before before_idx in doc.
    new_paras_data is a list of dicts: {style, text, font_size, bold, alignment}
    Returns the index of the last inserted paragraph (for reference).
    """
    body = doc.element.body
    target_element = paras[before_idx]._element
    
    last_inserted_idx = before_idx
    
    for i, pdata in enumerate(new_paras_data):
        # Create a new paragraph element
        new_p = docx.oxml.OxmlElement('w:p')
        body.insert(list(body).index(target_element), new_p)
        ...
        if pdata.get('text') is not None:
            ...
            t_elem.text = pdata['text']
            t_elem.set(docx.oxml.ns.qn('xml:space'), 'preserve')
            run_elem.append(t_elem)
        
        # Set style
        if pdata.get('style'):
            style_elem = docx.oxml.OxmlElement('w:pStyle')
            style_elem.set(docx.oxml.ns.qn('w:val'), pdata['style'])
            new_p.insert(0, style_elem)
    
    # Reload paragraph list
    return last_inserted_idx


# ============================================================
# Helper: Replace a range of paragraph texts
# ============================================================
def replace_paragraphs(doc, start_idx, end_idx, new_paras_data):
    """
    Replace paragraphs from start_idx to end_idx (inclusive) with new paragraphs.
    new_paras_data: list of {style, text}
    """
    body = doc.element.body
    ...
    # Insert new paragraphs at the position of start_idx
    # Find what's now at position start_idx (the element that was after end_idx)
    ref_paras = doc.paragraphs
    if start_idx < len(ref_paras):
        ref_element = ref_paras[start_idx]._element
    else:
        ref_element = body[-1]
    
    for pdata in reversed(new_paras_data):
        new_p = docx.oxml.OxmlElement('w:p')
...
# ============================================================
# Helper: Create a table from Markdown-like text
# ============================================================
def create_table_simple(doc, caption_text, rows_data, col_widths=None):
    """
    Create a table in document.
    rows_data: list of lists (first row is header)
    Returns the paragraph index after the table.
    """
    pass  # We'll handle tables via text-based approach


print("\n===== MODIFICATION 1: Insert 2.6 section before 2.5 本章小结 (para 379) =====")

# Build the new 2.6 section content
section_2_6 = [
    # H2: 2.6 汉字语义引擎：符号接地的跨模态扩展
...
# Insert before para 379 (H2 "2.5 本章小结")
# Using a different approach: manipulate oxml elements directly since inserting before is tricky
# Let's use a simpler approach: build the XML and insert

print("Building section 2.6 content...")
print(f"    Total paragraphs to insert: {len(section_2_6)}")

# We'll use ElementTree-level manipulation
from docx.oxml import OxmlElement
from docx.oxml.ns import qn
from lxml import etree

body = doc.element.body
target_para_379 = paras[379]._element
target_index_in_body = list(body).index(target_para_379)

print(f"    Target paragraph 379 index in body elements: {target_index_in_body}")

# Insert all paragraphs before target_para_379
for pdata in section_2_6:
    new_p = OxmlElement('w:p')
    body.insert(target_index_in_body, new_p)
    ...
    if pdata.get('style'):
        ...
        style_elem.set(qn('w:val'), pdata['style'])
        new_p.insert(0, style_elem)

print("    Section 2.6 inserted successfully!")

# Save intermediate state
print("Saving intermediate state...")
doc.save('易理研物（初稿）-temp.docx')
