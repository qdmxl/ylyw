...
svg = open(os.path.join(BASE, 'cover4_template.svg'), encoding='utf-8').read()
svg = svg.replace('__BRAIN_RING__', BRAIN)
out = os.path.join(BASE, 'cover4.svg')
open(out, 'w', encoding='utf-8').write(svg)
print('wrote', out)
import cairosvg
cairosvg.svg2png(url=out, write_to=os.path.join(BASE, 'cover4_hires.png'),
                 output_width=2480, output_height=3508)
cairosvg.svg2png(url=out, write_to=os.path.join(BASE, 'cover4_full.png'),
