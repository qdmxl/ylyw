...
svg = open(os.path.join(BASE, 'cover6_template.svg'), encoding='utf-8').read()
svg = svg.replace('__BRAIN_RING__', BRAIN).replace('__RAYS__', ''.join(RAYS))
out = os.path.join(BASE, 'cover6.svg')
open(out, 'w', encoding='utf-8').write(svg)
print('wrote', out)
import cairosvg
cairosvg.svg2png(url=out, write_to=os.path.join(BASE, 'cover6_hires.png'),
                 output_width=2480, output_height=3508)
cairosvg.svg2png(url=out, write_to=os.path.join(BASE, 'cover6_full.png'),
