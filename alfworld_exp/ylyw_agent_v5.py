#!/usr/bin/env python3
"""
YLYW Agent V5 — Admissible-Commands-Driven + YLYW Semantic Prior

核心改进（相比V4 baseline 3.7%）：
1. 从 task_desc 解析目标物体/工具/目标容器（英文直接匹配）
2. 利用 admissible_commands 作为状态信号：
   - take 命令 → 当前位置有什么物体
   - clean/heat/cool/use 命令 → 可以执行操作
   - put/move 命令 → 可以放置
3. 阶段自动推进：基于实际 admissible 信号而非盲猜
4. 系统性探索：遍历所有位置寻找目标物体，避免死循环
5. 多子目标任务的状态机正确实现
"""
...
class YLYWAgentV5:
    """
    YLYW Agent V5: admissible-commands 驱动的智能体
    """
    ...
    def reset(self, task_desc: str, task_type: str, pddl_params: Dict = None,
        ...
        # 从 pddl_params 优先提取目标（最准确）
        self._extract_targets_from_pddl()
    ...
    def _extract_targets_from_pddl(self):
        """从PDDL参数提取目标（最准确）"""
        if not self.pddl_params:
            return

        obj_target = self.pddl_params.get('object_target', '')
        parent_target = self.pddl_params.get('parent_target', '')
        mrecep_target = self.pddl_params.get('mrecep_target', '')

        if obj_target:
            # PDDL中的名字如 "Plate" → 小写 "plate"
            self.target_objects = [obj_target.lower()]

        if parent_target:
            self.target_receps = [parent_target.lower()]

        if mrecep_target:
            # movable receptacle
            self.target_receps.insert(0, mrecep_target.lower())
    ...
    def update(self, action: str, obs: str, info: Dict):
        ...
        # 跟踪手持物品
        if action.startswith('take ') and info.get('action_success', True):
        ...
        if (action.startswith('put ') or action.startswith('move ')) and info.get('action_success', True):
            self.holding = None
    ...
    def _act_put(self, cmds: List[str], obs: str) -> str:
        ...
        # 放置不可用 → 需要去目标容器
        if self.verbose:
            print(f"    [put] put action not available, going to recep")
        self.phase -= 1
        return self._act_find(cmds, 'find_recep', obs)
