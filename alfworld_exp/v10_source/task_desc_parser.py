...
RECEP_ALIASES = {
    ...
    'microwave': 'microwave',
    'ottoman': 'ottoman',
    'safe': 'safe', 'vault': 'safe',
    'shelf': 'shelf', 'book shelf': 'shelf', 'bookshelf': 'shelf',
...
def _extract_objects(desc: str, task_type: str) -> List[str]:
    ...
    # 方向性解析：尝试从"put/place/move X on/in/to Y"结构中提取X
    # 优先识别动作的直接宾语（被操作的物体）
    action_obj = _extract_action_object(desc)
...
def _extract_action_object(desc: str) -> str:
    """从动作句式中提取被操作的物体短语（直接宾语）"""
    # 模式: "put/place/move/take A (on/in/to/from) B"
    # 我们要的是A（被操作物体）
    patterns = [
        # "put a heated apple next to the lettuce" → "a heated apple"
        r'(?:put|place|move|transfer|bring)\s+(?:a\s+)?(?:warmed?|heated?|chilled?|cold|clean(?:ed)?|cool(?:ed)?|dirty)?\s*(.+?)\s+(?:on|in|into|to|next to|beside|inside|back)',
        # "take the salt from/by/off X" → "the salt"
        r'(?:take|get|grab|pick up)\s+(?:the\s+|a\s+)?(.+?)\s+(?:from|off|out|by)',
        # "take the salt, put it" → "the salt" (comma-separated)
        r'(?:take|get|grab|pick up)\s+(?:the\s+|a\s+)?(.+?)\s*,',
        # "wash the X before" → "the X"
        r'(?:wash|clean|heat|cool|chill)\s+(?:the\s+|a\s+)?(?:dirty\s+)?(.+?)\s+(?:before|then|and|in the)',
    ]
    # 取第一个匹配（最早出现的通常是主动词的直接宾语）
    for pat in patterns:
        m = re.search(pat, desc, re.IGNORECASE)
        if m:
            result = m.group(1).strip().lower()
            # 过滤掉代词
            if result not in ('it', 'them', 'this', 'that', 'one'):
                return result
    return ''
...
def _extract_receps(desc: str, task_type: str) -> List[str]:
    ...
    # 方向性解析：优先从"to/on/in Y"中提取目标容器
    dest_text = _extract_destination(desc)
    ...
    if task_type == 'pick_clean_then_place_in_recep':
        tool_exclusions.add('sinkbasin')
    elif task_type == 'pick_heat_then_place_in_recep':
        tool_exclusions.add('microwave')
    elif task_type == 'pick_cool_then_place_in_recep':
        tool_exclusions.add('fridge')

    # 优先从目标短语中提取
    if dest_text:
        for alias in sorted_aliases:
            if alias in dest_text:
                rec = RECEP_ALIASES[alias]
                if rec not in tool_exclusions and rec not in receps:
                    receps.append(rec)
                    break

    # 如果方向性提取没结果，回退到全文倒序扫描（最后提到的容器更可能是目标）
    if not receps:
        # 倒序扫描：在"from X to Y"结构中，Y在后面
        for alias in sorted_aliases:
            if alias in desc:
                rec = RECEP_ALIASES[alias]
                if rec not in tool_exclusions and rec not in receps:
...
def _extract_destination(desc: str) -> str:
    """从句式中提取放置目标短语"""
    # 模式："... to/on/in/into/inside DESTINATION"
    # 要取最后一个介词短语作为目标
    patterns = [
        # "put X on/in/into Y" — Y是目标
        r'(?:put|place|move|transfer|bring|return|back)\s+.+?\s+(?:on|in|into|inside|to|back to|onto)\s+(?:the\s+|a\s+)?(.+?)(?:\.|$)',
        # "from X to Y" — Y是目标
        r'from\s+.+?\s+to\s+(?:the\s+|a\s+)?(.+?)(?:\.|$)',
        # "X to put it on/in Y"
        r'to\s+put\s+it\s+(?:on|in|into)\s+(?:the\s+|a\s+)?(.+?)(?:\.|$)',
    ]
    # 取最后一个匹配（最可能是目标容器）
    best = ''
    best_pos = -1
